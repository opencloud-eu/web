...
- [x] **Phase 3 (first half)**: copied sidecar to `dev/docker/hocuspocus/`, appended `hocuspocus` service to `docker-compose.yml` (reuses `/realtime` Traefik path on the opencloud entrypoint, same env contract as web-extensions). `docker compose config` validates. Still to land: switch the etag probe inside `server.js` from WebDAV HEAD to Graph `/items/{id}` (separate commit; investigate the "personal-drive 400" mentioned in the original WebDAV-fallback comment).
