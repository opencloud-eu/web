...
- [x] **Phase 4.5**: `appWrapperEtagSyncKey` Vue inject contract exported from `AppTemplates/types.ts`. AppWrapper `provide()`s the setter; CollaborativeWrapper `inject()`s (null when standalone) and pushes every `_oc_meta.etag` change through it. Peer-saved etags reach AppWrapper.currentETag → next Ctrl+S / autosave no longer 412s with stale `previousEntityTag`. Idempotent setter so own-save mirror round-trip is a no-op. Codemirror + tiptap 9/9 e2e green. Cross-peer Cucumber scenario deferred to Phase 5 test migration.
