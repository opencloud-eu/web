...
- A running OpenCloud backend (root [docker-compose.yml](https://github.com/opencloud-eu/web/blob/main/docker-compose.yml), `docker-compose up -d`)
...
Available [projects](https://github.com/opencloud-eu/web/blob/main/tests/e2e/playwright.config.ts): `chromium`, `firefox`, `webkit`, `mobile-chromium`, `mobile-webkit`,
`ipad-chromium`, `ipad-landscape-webkit`.

Useful env vars (set before the command, e.g. `OC_BASE_URL=... pnpm test:e2e`; full list in
[`playwright.config.ts`](https://github.com/opencloud-eu/web/blob/main/tests/e2e/playwright.config.ts) `appConfig`):
...
(recommended in [`.vscode/extensions.json`](https://github.com/opencloud-eu/web/blob/main/.vscode/extensions.json)).
