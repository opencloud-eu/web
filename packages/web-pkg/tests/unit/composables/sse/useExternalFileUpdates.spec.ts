...
describe('useExternalFileUpdates', () => {
  ...
  it('drops events while inactive', async () => {
  ...
  it('drops a queued event once the wrapper turned inactive', async () => {
    const s = setup()

    s.emit(MESSAGE_TYPE.POSTPROCESSING_FINISHED, event({ etag: 'etag-2' }))
    await flushPromises()
    s.emit(MESSAGE_TYPE.POSTPROCESSING_FINISHED, event({ etag: 'etag-3' }))
    s.isActiveRef.value = false
    await s.finishHandler()

    expect(s.onExternalEvent).toHaveBeenCalledTimes(1)
  })

  it('drops a queued event when the resource changes', async () => {
    const s = setup()
