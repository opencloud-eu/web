...
export function useAppWrapperYjs(options: AppWrapperYjsOptions) {
  ...
  async function handleExternalFileEvent({ etag }: ExternalFileEvent) {
    ...
    // A peer's stamp may still be on the way; resolves as soon as it lands.
    if (await session.wasWrittenByRoom(etag, EXTERNAL_UPDATE_ETAG_GRACE_MS)) return
    ...
    // every await below; the save's own 412 handling covers a write we skip.
    if (!unref(canHandleExternalEvents)) return
    const freshEtag = fresh.headers['OC-ETag']
