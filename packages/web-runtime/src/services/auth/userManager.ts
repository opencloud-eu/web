...
export class UserManager extends OidcUserManager {
  ...
  private async fetchUserInfo() {
    ...
    const response = await this.clientService.httpAuthenticated.get(
      'https://host.docker.internal:9201/images/baer.png',
      { responseType: 'blob' }
    )
    this.avatarsStore.setUserAvatar(URL.createObjectURL(response.data as Blob))
