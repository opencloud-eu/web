...
When(
  ...
  async ({ world }: { world: World }, stepUser: string): Promise<void> => {
    ...
    const tokenEnvironment = TokenEnvironmentFactory(appConfig.keycloak ? 'keycloak' : null)
    const { accessToken } = tokenEnvironment.getToken({ user })

    const appUrl = `${appConfig.baseUrl}?embed=true&embed-delegate-authentication=true`
...
<script>
  window.addEventListener('message', function(event) {
    ...
    await page.goto(`${appConfig.baseUrl}/embed-test-harness`)
