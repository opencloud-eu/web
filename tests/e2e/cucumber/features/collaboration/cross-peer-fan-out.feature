Feature: Peer-save fan-out via _oc_meta
  ...
  Scenario: Alice saves, Brian sees the new state and his next save uses the fresh etag
    ...
    When "Alice" logs in
    And "Alice" opens file "fanout.md" via "code-mirror" using the context menu
    Then "Alice" should see the realtime collab status "connected"
    And "Brian" logs in
    And "Brian" navigates to the shared with me page
    And "Brian" opens file "fanout.md" via "code-mirror" using the context menu
    Then "Brian" should see the realtime collab status "connected"
    And "Brian" should see content "initial fan-out content" in the "codemirror" editor
    When "Alice" types "ALICE-SAVED" at the end of the "codemirror" editor
    Then "Brian" should see content "ALICE-SAVED" in the "codemirror" editor
    ...
    When "Brian" types "BRIAN-ADDED" at the end of the "codemirror" editor
